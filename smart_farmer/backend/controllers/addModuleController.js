...
const  createSensor= async (req,res)=>{
    ...
    const sensor_ids = (await farmModel.findOne({_id:farm_id},{sensors:1})).sensors
    console.log(name,port,sensor_ids)
    ...
    // console.log(exists_name,exists_port)
    try{
        if(exists_name){
        ...
        await farmModel.updateOne({_id:farm_id},{$push:{sensors:sensor._id}})
...
const  createActuator= async (req,res)=>{
    ...
    const actuator_ids = (await farmModel.findOne({_id:farm_id},{actuators:1})).actuators
    ...
    try{
        ...
        await farmModel.updateOne({_id:farm_id},{$push:{actuators:actuator._id}})
