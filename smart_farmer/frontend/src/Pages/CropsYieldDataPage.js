...
export default function CropYieldDataPage(props){
    ...
    useEffect(()=>{
        const fetchYieldData = async ()=>{
            ...
            const response = await fetch(`http://localhost:4000/api/cropyield/${farm_id}`)
            ...
            if(response.ok){
                ...
                console.log(temp_x)
                xAxisV.current = temp_x
                console.log(xAxisV.current)
                console.log(temp_dataSet)

                cropYieldData.current = temp_dataSet
                console.log(cropYieldData.current)
    ...
    return(
        ...
        {(cropYieldData.current!==[] && xAxisV.current!==[]) && (
        <BarChart
            yAxisLabel="Month"
            xAxisLabel="Yield (kg)"
            ...
            //             ]
            //         }
            xAxisValues = {xAxisV.current}
            dataSets = {cropYieldData.current}
            // xAxisValues = {x}
            // dataSets = {yd}
        />)}
