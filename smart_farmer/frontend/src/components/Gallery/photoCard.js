...
export default function PhotoCard({date,_id}){
    ...
    const handleDelete = async ()=>{
        const response = await fetch(`${process.env.REACT_APP_HOST}/api/photos/delete-photo/${_id}`,{
            method: "GET",
            crossDomain: true,
            headers: {
    ...
    return(
        <div class="col">
                    ...
                    <div class="card-body ">
                        ...
                        <button class="btn btn-danger btn-sm mt-3 ms-3" onClick={handleDelete}>Delete</button>
                    </div>
                    </div>
                </div>
